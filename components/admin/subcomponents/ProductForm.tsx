...
const ProductForm = () => {
  ...
  const handleImageChange = (i: number, v: string) => {
    const newColors = [...formData.productImage];
    newColors[i] = v;
    setFormData((prev) => ({ ...prev, productImage: newColors }));
  };
  ...
  const removeImageInput = (i: number) => {
    if (i == 0) return;
    setFormData((prev) => ({
      ...prev,
      productImage: prev.productImage.filter((_, a) => a !== i),
    }));
  };
